...
/*
 *  $Id: CgiUtils.cpp,v 1.2 2010/02/05 03:08:40 xxu Exp $
 *
...
std::string 
cgicc::dumpBuff(char * buffer, int length)
{	
	std::string result;
	...
	for(int i=0; i<length; i++) {
		result += charToHex(buffer[i]);
	}

	return(result);
}

char * cgicc::getIPaddr(unsigned long ip)
{	
	struct in_addr addr;
...
std::string 
cgicc::getMACaddr(unsigned char * mac)
{	
	std::string result;
...
std::string  cgicc::Replace(std::string   &str,   const   char   *string_to_replace,   const   char   *new_string)   
  {   
...
unsigned char*  cgicc::trans( const  char *src )    
{
     char c;
     int i;
     unsigned char temp;
...
const unsigned char*  cgicc::getMac(  std::string& mac_in, unsigned char array[] )
{		
		char field_name[32];
		std::string output = cgicc::Replace(mac_in, ":" ,"" );	
		
		//addUserVariable ("p",output );
		unsigned char *ptran = cgicc::trans( output.c_str() );
		//sprintf (field_name, "%d-%d-%d-%d-%d-%d",*ptran, *(ptran+1),*(ptran+2),*(ptran+3),*(ptran+4),*(ptran+5));
		...
		array[4] = ptran[4];
		array[5] = ptran[5];
		//sprintf (field_name, "%d-%d-%d-%d-%d-%d",array[0], array[1],array[2],array[3],array[4],array[5]);
		//addUserVariable ("p",field_name );
		//memcpy(array, ptran,6 * sizeof(unsigned char) ); why?
