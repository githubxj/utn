...
/*
 *  $Id: CgiUtils.h,v 1.2 2010/02/05 03:08:40 xxu Exp $
 *
...
namespace cgicc {
  ...
  std::string 
  dumpBuff(char * buffer, int length);

  char * getIPaddr(unsigned long);

  std::string getMACaddr(unsigned char * mac);

  std::string  Replace(std::string   &str,   const   char   *string_to_replace,   const   char   *new_string) ;  
unsigned char*  trans( const  char *src ) ;
const unsigned char*  getMac(  std::string& mac_in, unsigned char array[] );
